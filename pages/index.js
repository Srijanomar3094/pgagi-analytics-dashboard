...
export default function Home() {
  ...
  const weatherChartData = {
    labels: weather?.map(city => city.name) || [],
    datasets: [
      {
        label: 'Temperature (°C)',
        data: weather?.map(city => city.main.temp) || [],
        fill: false,
  ...
  const flattenedFinanceData = finance.flat();

  const financeChartData = {
    labels: flattenedFinanceData.map((stock) => stock.symbol) || [], 
    datasets: [
      {
        label: "Stock Price",
        data: flattenedFinanceData.map((stock) => stock.price) || [], 
        backgroundColor: flattenedFinanceData.map((stock) =>
          stock.change >= 0
            ? "rgba(34, 139, 34, 0.8)"
            : "rgba(220, 20, 60, 0.8)" 
        ),
        borderColor: flattenedFinanceData.map((stock) =>
          stock.change >= 0
            ? "rgba(34, 139, 34, 1)"
            : "rgba(220, 20, 60, 1)" 
        ),
    ...
    <DashboardLayout>
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-gray-600 dark:text-gray-300 mb-16">
          Get an overview of Weather and Finance Updates
        </p>
      </div>
      <main className="mb-2 p-2 space-y-6">
        <div className="flex justify-between gap-x-6">
         
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 flex-1 border border-gray-300 dark:border-gray-600">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-2">
        <div
          className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 flex flex-col items-center cursor-pointer transition-transform transform hover:scale-105 border border-gray-300 dark:border-gray-600"
          onClick={() => navigateTo('/weather')}
        >
          <div className="bg-yellow-500 text-white rounded-full p-6 mb-4">
            <Cloud size={24} />
          </div>
          <h2 className="text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-2">Weather</h2>
        ...
        >
          ...
          <h2 className="text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-2">News</h2>
        ...
        >
          ...
          <h2 className="text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-2">Finance</h2>
